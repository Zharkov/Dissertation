...
class ARIMA_NEMS:
    ...
    HYPERPARAMS = {
        ...
        "n_models":        6,
        "forecast_mode":   "static multi-step (без rolling update)",
    }
...
if __name__ == "__main__":
    ...
    from common import load_nems, split_train_test, evaluate, print_results_table

    print("=" * 70)
    print(f"  ARIMA(1,0,0) (StatsModels) — НЭМС")
    print("=" * 70)
    ...
    model = ARIMA_NEMS(lag=1, order_d=0, order_q=0)
    model.fit(X_train)

    preds = model.rolling_forecast(X_test)
    result = evaluate("ARIMA", X_test, preds)
    print_results_table([result], "Результат ARIMA — НЭМС")
