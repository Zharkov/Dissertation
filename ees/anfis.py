...
def subtractive_clustering(X: np.ndarray, r_a: float = 0.25,
...
class ANFIS_EES:
    ...
    def __init__(self, lag: int = 4, n_concepts: int = 4,
                 r_a: float = 0.30, lr: float = 0.003,
...
if __name__ == "__main__":
    ...
    from common import load_ees, split_train_test, evaluate, print_results_table, LAG

    print("=" * 70)
    print(f"  ANFIS (PyTorch, обучение на разностях) — ЭЭС (LAG={LAG})")
    print("=" * 70)
    ...
    model = ANFIS_EES(lag=LAG, n_concepts=4)
    model.fit(X_train)
    ...
    print_results_table([result], "Результат ANFIS — ЭЭС")
