...
class LSTMNet(nn.Module):
    ...
    def __init__(self, n_features: int = 4, hidden_size: int = 64, num_layers: int = 2):
        super().__init__()
        self.lstm = nn.LSTM(
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(f"  LSTM (PyTorch, обучение на разностях) — ЭЭС (LAG={LAG})")
    print("=" * 70)
    ...
    model = LSTM_EES(lag=LAG, n_concepts=4)
    model.fit(X_train)
    ...
    print_results_table([result], "Результат LSTM — ЭЭС")
